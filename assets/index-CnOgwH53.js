...
Squat 3x12 80kg" style="width: 100%; height: 150px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: #fff; padding: 12px; font-family: inherit; resize: none"></textarea>
  ...
  `,document.body.appendChild(n),document.getElementById(`modal-pause`).addEventListener(`click`,()=>{n.remove(),t&&t()}),document.getElementById(`modal-interrupt`).addEventListener(`click`,()=>{n.remove(),e&&e()}),document.getElementById(`modal-cancel`).addEventListener(`click`,()=>{n.remove()})};J(`dashboard`),document.addEventListener(`click`,O,{once:!0}),document.addEventListener(`touchstart`,O,{once:!0}),[`visibilitychange`,`pageshow`,`focus`].forEach(e=>{window.addEventListener(e,()=>{document.visibilityState===`visible`&&typeof y==`function`&&y()})});