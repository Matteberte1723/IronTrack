...
Squat 3x12 80kg" style="width: 100%; height: 150px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: #fff; padding: 12px; font-family: inherit; resize: none"></textarea>
  ...
  `,document.getElementById(`back-to-history`).addEventListener(`click`,()=>X()),document.getElementById(`return-history`).addEventListener(`click`,()=>X())},Z=()=>{let e=()=>{n.innerHTML=`
    ...
    `,document.getElementById(`close-settings`).addEventListener(`click`,()=>e()),document.getElementById(`edit-profile-btn`).addEventListener(`click`,()=>p()),document.getElementById(`unit-kg`).addEventListener(`click`,()=>{f.unit=`kg`,t.saveUser(f),m()}),document.getElementById(`unit-lbs`).addEventListener(`click`,()=>{f.unit=`lbs`,t.saveUser(f),m()}),document.getElementById(`alarm-toggle`).addEventListener(`change`,e=>{t.saveAlarmEnabled(e.target.checked),m()}),document.querySelectorAll(`.alarm-sound-option`).forEach(e=>{e.addEventListener(`click`,n=>{if(n.target.closest(`.preview-sound-btn`))return;let r=e.getAttribute(`data-sound`);t.saveAlarmSound(r),I(),m()})}),document.querySelectorAll(`.preview-sound-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),R(e.getAttribute(`data-sound`))})}),document.querySelectorAll(`.alarm-duration-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let n=parseInt(e.getAttribute(`data-duration`));t.saveAlarmDuration(n),m()})}),document.querySelectorAll(`.theme-circle`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.target.getAttribute(`data-theme`);t.saveTheme(n),i(n),m()})}),document.getElementById(`export-btn-settings`).addEventListener(`click`,a),document.getElementById(`import-input-settings`).addEventListener(`change`,e=>{e.target.files.length>0&&o(e.target.files[0])})};e()},se=()=>{n.innerHTML=`
    <div class="view" style="padding: 20px">
